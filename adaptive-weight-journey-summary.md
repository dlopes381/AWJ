...
- **Quick weight log** (`quickLogWeight`, dashboard-only): a one-tap weight input above "+ Log full activity" for the single most common daily action, without opening the full Log tab form. Preserves that day's existing notes (if any) rather than blanking them, since the quick form has no notes field. Shares the same outlier guard and phase/goal-crossing detection as the full save flow.
