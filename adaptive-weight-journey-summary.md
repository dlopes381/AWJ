...
- **Quick log card** (`quickLogEntry`, dashboard-only): a single "Today, `<date>`" card with a weight field and a calories field and one "Log today's entry" / "Update today's entry" button (label and a small "Already logged — editing" note switch based on whether today already has a weight or nutrition entry) — always scoped to today's date, no date picker, since that's what "+ Log full activity" is for. Both fields prefill with today's current values when present. Weight replaces today's weigh-in (same outlier guard and phase/goal-crossing detection as the full save flow, preserving existing notes). The calorie field only touches today's nutrition log if its value actually differs from today's current total — so re-saving the unchanged prefilled total (e.g. because the user only meant to update weight) can't silently collapse an itemized multi-meal day into a single line; when it *is* changed, it replaces the day's meals with one "Other"-labeled item for that amount, preserving any existing protein/carbs/fat/water. (Earlier versions had two separate weight/calorie mini-forms; consolidated into one per user feedback that logging is one action per day, not two.)
