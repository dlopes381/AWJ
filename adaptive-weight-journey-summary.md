...
- **Quick weight + calorie log** (`quickLogWeight`, `quickLogCalories`, dashboard-only): one-tap weight and calorie inputs above "+ Log full activity" for the two most common daily actions, without opening the full Log tab form. `quickLogWeight` preserves that day's existing notes (if any) rather than blanking them, and shares the same outlier guard and phase/goal-crossing detection as the full save flow. `quickLogCalories` appends a single "Other"-labeled meal line item to today's nutrition log (creating the day's entry if needed) rather than requiring the itemized per-meal form.
